...
//       SendDlgItemMessage(hDlg,ID_RBTN1 + j,BM_SETCHECK,1,0);
    ...
    case WM_COMMAND:
        ...
        case IDCANCEL:
            PostMessage(hDlg,WM_CLOSE,0,0);
            break;
        case ID_RBTN5: /* disassembler */
            PostMessage(GetParent(hDlg),WM_COMMAND,ID_SUBDLG6,0);
            PostMessage(hDlg,WM_COMMAND,IDCANCEL,0); /* close the "hexdump" window */
            break;
        case ID_RBTN1:
        case ID_RBTN2:
        case ID_RBTN3:
...
BOOL EXPORTED CALLBACK ViewDlg(HWND hDlg,UINT message,WPARAM wParam,LPARAM lParam )
{
    ...
    case WM_COMMAND:
        ...
        case ID_REFRESH:
            ...
            TRACEOUT("ViewDlg: ID_REFRESH, SendMessage HD_RESETCONTENT to HexDump Control");
            if (!SendDlgItemMessage(hDlg,ID_LISTBOX1,HD_RESETCONTENT,0,(LPARAM)(LPSTR)lpw)) {
                rc = TRUE;
                break;
            }
            ;
            SendDlgItemMessage(hDlg,ID_STATUS1,ST_SETTABSTOPS,LoadTabs(IDUS_36,str),(LPARAM)(LPVOID)str);
...
#ifdef _TRACE_
            wsprintf(str,"ViewDlg.ID_SubDlg6: fw.offset=%lX fw.selector=%X\r\n",fw.offset,fw.selector);
